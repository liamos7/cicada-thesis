...
class CICADA(VisionDataset):
    ...
    Assumes data is stored in HDF5 format with 'et' dataset for images
    """
    # def __init__(self, root: str, train: bool = True, transform=None, download: bool = False):
    def __init__(self, root: str = "/eos/user/l/ligerlac/cicada_data/", transform=None, train=True, **kwargs):
        self.root = root
        ...
        self.class_dict = {
            "ZeroBias_EvenLumi": 0,
            "ZeroBias_OddLumi": 0,   # same class
            "SUSYGluGluToBBHToBB_NarrowWidth_M-350_TuneCP5_13p6TeV-pythia8_0": 1,
            "VBFHToTauTau_M125_TuneCP5_13p6TeV_powheg-pythia8_1": 2,
        }
    ...
    def _load_data(self):
        ...
        for process, label in self.class_dict.items():
            data_path = self.root + f"{process}.h5"
            with h5py.File(data_path, 'r') as f:
                X.append(f['CaloRegions'][:])
                y.append(np.full(f['CaloRegions'].shape[0], label))
