...
class AE(nn.Module):
    ...
    def validation_step(self, x, y, **kwargs):
        ...
        d = {
            "loss": predict[y==0].mean().item(),
            "loss/loss_holdout_": predict[y==1].mean().item(),
            "loss/recon_error_": ((recon - x) ** 2)[y==0].mean().item(),
            "loss/recon_error_holdout_": ((recon - x) ** 2)[y==1].mean().item(),
            "predict": predict,
            "reconstruction": recon,
        }
        if kwargs.get('show_image', True):
            d.update({
                "input/input@": make_grid(x[y==0].detach().cpu(), nrow=10, value_range=(0, 1)),
                "recon/recon@": make_grid(recon[y==0].detach().cpu(), nrow=10, value_range=(0, 1)),
                "input/input_holdout@": make_grid(x[y==1].detach().cpu(), nrow=10, value_range=(0, 1)),
                "recon/recon_holdout@": make_grid(recon[y==1].detach().cpu(), nrow=10, value_range=(0, 1)),
            })
...
class NAEWithEnergyTraining(NAE):
    """
    Extended NAE with energy-based training capabilities.
    
    This implements a simplified version of the energy-based (Phase 2) training
    from the reference implementation, using basic contrastive divergence.
    """
    ...
    def __init__(
        ...
        # Data space Langevin parameters
        # x_noise_std=0.05,
        x_noise_std=0.001,
        x_steps=10,
        x_step_size=0.5,

        # Latent space Langevin parameters
        # z_noise_std=0.02,
        z_noise_std=0.01,
        z_steps=10,
        z_step_size=0.2,
        ...
        """
        Args:
            gamma: Regularization weight for negative energy
            x_noise_std: Standard deviation for Langevin noise in data space
            x_steps: Number of Langevin steps in data space
            x_step_size: Step size for Langevin dynamics in data space
            z_noise_std: Standard deviation for Langevin noise in latent space
            z_steps: Number of Langevin steps in latent space
            z_step_size: Step size for Langevin dynamics in latent space
            use_two_stage: If True, use two-stage sampling (latent then data space)
        """
        super().__init__(encoder, decoder, spherical, temperature, temperature_trainable)
        
        # Data space sampling parameters
        self.x_noise_std = x_noise_std
        self.x_steps = x_steps
        self.x_step_size = x_step_size
        
        # Latent space sampling parameters
        ...
        self.gamma = gamma  # Regularization coefficient for negative energy


    def load_pretrained_nae(self, model_path, **kwargs):
        """
        Load state dict from a pretrained NAE model.

        Args:
            model_path: Path to pretrained NAE checkpoint
            encoder: New encoder instance (same architecture as pretrained)
            decoder: New decoder instance (same architecture as pretrained)
            device: Device to load model on
            **kwargs: Additional arguments for NAEWithEnergyTraining constructor
        """
        ...
        # Load compatible weights (ignore energy-training specific parameters)
        missing_keys, unexpected_keys = self.load_state_dict(pretrained_state, strict=False)
        
        # Filter out expected missing keys (energy training parameters)
        energy_specific_keys = ['x_noise_std', 'x_steps', 'x_step_size', 'z_noise_std', 
                               'z_steps', 'z_step_size', 'use_two_stage', 'neg_lambda', 'gamma']
        
        critical_missing = [k for k in missing_keys if not any(param in k for param in energy_specific_keys)]
        
        if critical_missing:
            print(f"Warning: Critical keys missing: {critical_missing[:5]}...")
        else:
            print("Successfully loaded pretrained NAE weights for energy training")


    def langevin_sample(self, x_init):
        """
        Generate negative samples using Langevin MCMC.
        
        Uses either single-stage (data space only) or two-stage sampling
        (latent space initialization + data space refinement).
        """
        if self.use_two_stage:
            return self.two_stage_langevin_sample(x_init)
        else:
            return self.single_stage_langevin_sample(x_init)
        
    
    def single_stage_langevin_sample(self, x_init):
        """Single-stage Langevin sampling in data space only."""
        ...
        for _ in range(self.x_steps):
            ...
            #print(f"[Data Langevin] Step {_}, energy: {energy.item():.6f}")
            
            # Langevin update with noise
            with torch.no_grad():
                noise = torch.randn_like(x) * self.x_noise_std
                x = x - self.x_step_size * grad + noise
                x = torch.clamp(x, 0, 1)
    ...
    def two_stage_langevin_sample(self, x_init):
        """
        Two-stage Langevin sampling with On-Manifold Initialization (OMI).
        
        Stage 1: Sample in latent space Z
        Stage 2: Refine in data space X starting from decoder(z_sample)
        """
        batch_size = x_init.shape[0]
        
        # Stage 1: Sample in latent space
        # Get latent dimension by encoding a sample
        with torch.no_grad():
            z_init = self.encode(x_init[[0]])  # Use first sample to get shape
            z_shape = z_init.shape[1:]
        
        # Initialize latent samples
        z = torch.randn(batch_size, *z_shape, device=x_init.device, requires_grad=True)
        if self.spherical:
            z = self.normalize(z.detach()).requires_grad_(True)
        
        # Langevin sampling in latent space
        # print(f"Starting Langevin sampling in latent space for {self.z_steps} steps")
        for _ in range(self.z_steps):
            # Energy function for latent space: E(x) where x = decoder(z)
            x_from_z = self.decode(z)
            energy = self.energy(x_from_z).sum()
            #print(f"Step {_}, Latent energy: {energy.item()}")
            grad = torch.autograd.grad(energy, z, retain_graph=False)[0]
            
            with torch.no_grad():
                noise = torch.randn_like(z) * self.z_noise_std
                z = z - self.z_step_size * grad + noise
                
                # Apply spherical constraint if needed
                if self.spherical:
                    z = self.normalize(z)
                    
            z = z.detach().requires_grad_(True)
        
        # Stage 2: Initialize data space from latent samples
        with torch.no_grad():
            x_init_from_z = self.decode(z)
            x_init_from_z = torch.clamp(x_init_from_z, 0, 1)
        
        # Refine in data space starting from manifold initialization
        x = x_init_from_z.requires_grad_(True)

        # print(f"Starting Langevin sampling in data space for {self.x_steps} steps")
        for _ in range(self.x_steps):
            energy = self.energy(x).sum()
            #print(f"Step {_}, Data space energy: {energy.item()}")
            grad = torch.autograd.grad(energy, x, retain_graph=False)[0]
    ...
    def train_step(self, x, optimizer, clip_grad=None, **kwargs):
        """
        Energy-based training step using contrastive divergence.
        This implements Phase 2 training from the reference.
        """
        optimizer.zero_grad()
        ...
        # Negative samples via Langevin sampling
        # The Langevin sampling process itself handles gradient detachment internally
        x_neg = self.langevin_sample(x)
        neg_energy = self.energy(x_neg)

        # Contrastive divergence loss
        loss = pos_energy.mean() - self.neg_lambda * neg_energy.mean()

        # Optional regularization on negative energy
        if self.gamma is not None:
            loss = loss + self.gamma * (neg_energy ** 2).mean()
        ...
        return {
            ...
            'energy/temperature_': self.temperature.item()
        }
    
    def validation_step(self, x, y, **kwargs):
        """
        Validation step for energy-based training.
        Uses standard reconstruction-based validation without energy sampling.
        """
        # Just use the base validation (reconstruction metrics and ROC-AUC)
