...
def get_cicada_ae():
    encoder = CicadaEncoder(latent_dim=20)
    decoder = CicadaDecoder(latent_dim=20)
    model = AE(encoder, decoder)
    return model


def get_cicada_nae_with_energy(pretrained_path: str):
    encoder = CicadaEncoder(latent_dim=20)
    decoder = CicadaDecoder(latent_dim=20)
    model = NAEWithEnergyTraining(
        encoder,
...
def get_teacher_model(model_name: str, dataset_name: str, pretrained_path: str = None):
    ...
    if dataset_name == "MNIST":
        ...
        if model_name == "AE":
            ...
            model = get_cicada_nae_with_energy(pretrained_path)
        else:
